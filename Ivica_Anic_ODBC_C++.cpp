        ...
        else
        {
                ...
                pThisBinding = pThisBinding->sNext)
            {
                if (pThisBinding->indPtr != SQL_NULL_DATA)
                {
                    ...
                    _bstr_t temp(pThisBinding->wszBuffer);
                    const char* chartemp = temp;
                    char src[50], dest[50];
                    strcpy_s(src, chartemp);

                    if (i == 0)
                    {
                        strcpy_s(AdressenData, src);
                        strcat_s(AdressenData,  " ");
                    }
                    else
                    {
                        strcat_s(AdressenData, src);
                        strcat_s(AdressenData, " ");
                    }
